...
import {
  ...
  Wallet, CheckCircle2, User, Loader2, Users, FileText, Pill, CalendarPlus
} from "lucide-react";
    ...
    <div className="flex flex-col gap-8 animate-fade-in pb-12 max-w-[1700px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between px-2 gap-6">
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900">Hospital Operations</h1>
        </div>
      ...
      <div className="rounded-[2.5rem] bg-gradient-to-r from-slate-900 to-slate-950 p-10 flex flex-col xl:flex-row xl:items-center justify-between gap-8 border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          <div className="h-20 w-20 rounded-[1.5rem] bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center shadow-lg shadow-cyan-500/30 shrink-0">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-400 mt-2 max-w-lg leading-relaxed">
        ...
        <div className="flex flex-wrap items-center gap-4 relative z-10">
          ...
          <div className="flex gap-3">
            {supportContacts.map((creator) => (
              <a key={creator.name} href={getWhatsappLink(creator.whatsappNumber, creator.message)} target="_blank" rel="noreferrer" className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-3 pr-6 hover:bg-white/10 transition-all shrink-0">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">{creator.role}</p>
                </div>
              </a>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { title: "Register Patient", desc: "Front desk intake", icon: Users, href: "/patients", color: "text-blue-600", bg: "bg-blue-50" },
          { title: "Consultations", desc: "Active doctor queue", icon: Stethoscope, href: "/consultation", color: "text-emerald-600", bg: "bg-emerald-50" },
          { title: "Pharmacy POS", desc: "OTC & Prescriptions", icon: Pill, href: "/pharmacy/otc-sales", color: "text-purple-600", bg: "bg-purple-50" },
          { title: "Billing Desk", desc: "Invoices & Cashier", icon: Wallet, href: "/billing", color: "text-amber-600", bg: "bg-amber-50" },
        ].map((action) => (
          <Link key={action.title} href={action.href} className="rounded-2xl glass panel-shadow p-5 flex items-center gap-4 hover:-translate-y-1 transition-transform border border-white/60 bg-white/40">
             ...
             <div>
               ...
               <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mt-0.5">{action.desc}</p>
             </div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-8">
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-[2.5rem] glass panel-shadow p-8 md:p-10 bg-white/60 border border-white/60">
            <div className="flex items-center justify-between mb-8">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Live Tracker</p>
                <h2 className="text-2xl font-black text-slate-800 mt-1">Active Module Operations</h2>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
              {[
                ...
                <div key={String(label)} className="bg-white border border-slate-100 rounded-[1.5rem] p-6 shadow-sm hover:-translate-y-1 transition-transform">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{label}</p>
                  <p className={cn("text-4xl font-black mt-2", colorClass || "text-slate-800")}>{value}</p>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
                <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Recent Submissions Log</p>
                <Link href="/reports" className="text-[11px] font-bold uppercase tracking-wider text-cyan-600 hover:text-cyan-800">View Complete Ledger</Link>
              </div>

              {moduleOperations.isLoading ? <Loader2 className="h-6 w-6 animate-spin text-slate-300 mx-auto my-8" /> : recentModuleRecords.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                  ...
                  <p className="text-sm font-bold text-slate-500">No module records actively tracking.</p>
                  <p className="text-xs font-medium text-slate-400 mt-1">Submissions will appear here once workflows begin.</p>
                </div>
              ) : recentModuleRecords.slice(0, 6).map(record => (
                <Link key={record.id} href={`/${record.moduleSlug}`} className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white border border-slate-100 rounded-2xl hover:border-cyan-300 hover:shadow-md transition-all group gap-4">
                  <div className="min-w-0">
                    <div className="flex gap-2 mb-2">
                      <Badge className="bg-slate-100 text-slate-600 shadow-none border-0 text-[9px] font-bold uppercase tracking-wider">{record.moduleTitle}</Badge>
                      <Badge variant="outline" className="shadow-none border-slate-200 text-slate-500 text-[9px] font-bold uppercase tracking-wider">{record.statusCode}</Badge>
                    </div>
                    <p className="text-base font-bold text-slate-800 truncate group-hover:text-cyan-700 transition-colors">{record.title}</p>
                    <p className="text-[11px] font-bold text-slate-400 mt-1.5 uppercase tracking-wider">{record.recordNumber} • Stage: {record.workflowStage}</p>
                  </div>
                  <div className="text-left sm:text-right shrink-0 bg-slate-50 sm:bg-transparent p-4 sm:p-0 rounded-xl">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Target Deadline</p>
                    <div className="flex items-center gap-1.5 justify-start sm:justify-end mt-1">
                      <Clock3 className="h-3 w-3 text-slate-400" />
                      <p className="text-sm font-bold text-slate-700">{formatDate(record.dueAt)}</p>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <div className="rounded-[2.5rem] glass panel-shadow p-8 md:p-10 bg-white/60 border border-white/60">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h2 className="text-2xl font-black text-slate-800 mt-1">Operational Pulse</h2>
              </div>
              <div className="h-12 w-12 rounded-2xl bg-slate-100 flex items-center justify-center">
        ...
        <div className="space-y-8 flex flex-col">
          ...
          {/* Active Session Card */}
          <div className="rounded-[2.5rem] glass panel-shadow p-8 bg-slate-900 border border-slate-800 text-white shrink-0 shadow-xl shadow-slate-900/20">
            <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Active Security Session</p>
            ...
            <div className="space-y-3">
              <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-4">
                 <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Assigned Role</p>
                 <p className="text-sm font-bold text-cyan-400 mt-1">{user?.roleCode || "—"}</p>
              </div>
              <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-4">
                 <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">System Scope Visibility</p>
