...
export class BillingService {
    ...
    if (this.mpesaActivePrompts >= maxConcurrent) {
      await new Promise<void>((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          const index = this.mpesaPromptQueue.indexOf(queued);
  ...
  async closeInvoice(id: number, user: RequestUser) {
    ...
    if (Number(invoice.balanceAmount) > 0) {
      throw new BadRequestException(
        `Invoice cannot be closed while ${String(invoice.balanceAmount)} remains unpaid.`,
      );
    }
  ...
  async createInvoice(dto: CreateInvoiceDto, user?: RequestUser) {
    ...
    return {
      ...invoice,
      verificationCode:
        this.billingDocumentService.buildVerificationCode(invoice),
    };
  }
  ...
  async getInvoicePdf(id: number, user: RequestUser) {
    const invoice = await this.getInvoiceByIdScoped(id, user);
    return this.billingDocumentService.createInvoicePdf(invoice);
  }
  ...
  async getPaymentReceiptPdf(id: number, user: RequestUser) {
    ...
    return this.billingDocumentService.createPaymentReceiptPdf(payment);
  }
  ...
  async createCashPayment(dto: CreateCashPaymentDto, user?: RequestUser) {
    ...
    if (dto.amount > Number(invoice.balanceAmount)) {
      throw new BadRequestException(
        `Payment exceeds outstanding balance of ${String(invoice.balanceAmount)}`,
      );
    }
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (reserved.count !== 1) {
          ...
          throw new BadRequestException(
            `Payment exceeds outstanding balance of ${String(current?.balanceAmount ?? 0)}`,
          );
        }
        ...
        await tx.notification.create({
          data: {
            ...
            message: `Cash payment of ${String(payment.amount)} received for invoice ${payment.invoiceId}.`,
    ...
    try {
      const outcome = await this.prisma.$transaction(async (tx) => {
        ...
        if (reserved.count !== 1) {
          ...
          throw new BadRequestException(
            `M-PESA payment exceeds outstanding balance of ${String(current?.balanceAmount ?? 0)}`,
          );
        }
