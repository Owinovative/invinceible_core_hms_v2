...
| Requirement | Reason |
| --- | --- |
| Full virtual-claim workflow | DHA requires a stateful visit/consent, intervention, diagnosis, billable line, attachment, preview, submit, discharge/close workflow. `ShaClaim` only stores a local claim summary and does not persist DHA consent tokens, intervention/line/attachment identifiers, visit GUIDs, preauth data or workflow transitions. Sending its generic FHIR Bundle would be noncompliant, so the adapter now rejects that unsafe submission path. |
