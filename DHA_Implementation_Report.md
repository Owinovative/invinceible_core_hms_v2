...
| DHA command workflow | Added a queue-backed, DTO-validated DHA command contract for the documented JSON API: virtual claim/consent lifecycle, claim mutation and preview, authorization lookup, tariff resolution, preauth lookup/cancellation and clinician/diagnosis removal, prescription preview/create/dispense/clinician removal, emergency claim/protocol/doctor actions, occupancy, benefits/utilization/POMSF reads, OTP-whitelist callbacks, and presigned DHA file URL retrieval. Each command uses the documented endpoint and required fields, carries facility context, is idempotently queued, and is recorded in the existing DHA transaction and audit trail. Legacy FHIR Claim transactions now fail closed rather than being sent to an undocumented endpoint. | `backend/src/integration/dha/eclaims-contract.ts`, `backend/src/integration/dha/dha.service.ts`, `backend/src/integration/dha/dha.controller.ts` |
| Multipart transport safety | The shared integration HTTP client now transmits `FormData` without forcing an `application/json` content type, allowing fetch to provide the required multipart boundary for DHA file-based workflows. | `backend/src/integration/http/integration-http.client.ts` |
| Consent-token protection | Queued DHA consent-token commands require a local patient context. The token is removed before persistence and restored from the active local authorization only at worker execution time, preventing bearer-equivalent consent tokens from being retained in transaction or queue JSON. | `backend/src/integration/dha/dha.service.ts`, `backend/src/integration/dha/adapters/dha-http.client.ts` |
| Attachment encryption gate | DHA sandbox and production startup now requires a 32-byte AES-256 attachment encryption key (`DHA_ATTACHMENT_ENCRYPTION_KEY`) in addition to credentials and webhook secret, preventing a file-backed DHA workflow from being enabled without encryption material. | `backend/src/config/env.validation.ts`, `backend/src/integration/integration-config.service.ts`, `backend/.env.example` |
...
| File-backed workflows | The transport is multipart-safe, but the application does not yet have a durable encrypted attachment store and file-reference model capable of queueing DHA preauth creation, EMT claims, claim attachments, uploads, or OTP-whitelist submissions without storing raw file bytes in queue JSON. These workflows cannot be safely enabled until that persistence boundary exists. |
