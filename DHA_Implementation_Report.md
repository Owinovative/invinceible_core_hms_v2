...
| Full preauth and emergency/EMT workflow | The documented JSON preauth lifecycle, prescription/dispense, and emergency-protocol calls are supported by the command layer. Creating a preauth and EMT claims remain blocked because DHA specifies multipart/form-data with file uploads; the local clinical workflow also does not yet persist the DHA preauth/emergency identifiers and workflow transitions. |
