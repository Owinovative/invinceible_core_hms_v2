...
Invinceible Core HMS should run as multiple stateless backend instances behind a load balancer, one Vercel frontend deployment, one primary MySQL database, Redis for cache/rate limits/queues, and separate worker processes for heavy jobs.

## Production topology

- Frontend: Vercel CDN and edge cache.
- Backend API: two or more Railway replicas when traffic grows.
